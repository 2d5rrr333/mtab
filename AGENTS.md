...
- **当前状态**：v0.5.0，5 个 capability（clock-widget/bookmark-grid/search-box/configuration/countdown-widget）共 39 条需求规格，13 个 change 归档于 `openspec/changes/archive/`，已发布 mooncakes.io（`2d5rrr333/mtab`）。mtab 是两个生态包的参考应用：**桥协议层 = `2d5rrr333/moonbridge`**（泛型 Session + `web/vendor/moonbridge.mjs` 运行时，独立仓库 `D:\code\moonbit\moonbridge`）、**测试工具链 = `2d5rrr333/moonwebtest`**（`tools/` vendored，独立仓库 `D:\code\moonbit\moonwebtest`）。功能：时钟（时:分 + 农历/节气天文历算/节日徽章）、书签分组拖拽（Pointer Events 自实现，默认预置常用站点）、Netscape 书签 HTML 导入（容错解析/同名合并/URL 去重）、倒数日小组件（跨年闰年日期运算/跨日刷新）、搜索历史联想（trie+半衰期评分）+拼音联想（~2000 字表）、config v2（v1 自动迁移）、主题令牌化（深/浅/自动）、壁纸模糊度
- **验证基线**：`moon test` 162 / `node e2e.mjs` 63 / `node headless.cjs`（Chromium 系浏览器双运行）127 与 129 断言、渲染检查 13；改完必须全绿 + `moon fmt`/`moon info` 干净
